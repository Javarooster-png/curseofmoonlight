...
void DrawDebuffBar(PlayState* play, Player* player) {
    InterfaceContext* interfaceCtx = &play->interfaceCtx;
    s16 debuffBarY = SCREEN_HEIGHT - 44;    // Lower center of the screen
    s16 debuffBarX = SCREEN_WIDTH / 2 - 50; // Centered horizontally
    s16 debuffBarWidth = 100;               // Total width of the interior fill bar
    s16 debuffBarHeight = 16;               // Height of the bar (based on texture height)

    // Calculate the width of the shrinking bar based on PlayerWebTimer
    s16 filledWidth = debuffBarWidth * (player->PlayerWebTimer / 255.0f); // Shrink fill based on timer

    // Calculate color interpolation (white to black) based on PlayerWebTimer
    u8 colorValue = (u8)(255 * (player->PlayerWebTimer / 255.0f)); // Ensure this is correct

    OPEN_DISPS(play->state.gfxCtx, "../z_parameter.c", 3000);

    Gfx_SetupDL_39Overlay(play->state.gfxCtx); // Set up rendering pipeline for HUD overlay elements

    // --- Draw the static border (non-shrinking) ---
    // Left end of the border
    gDPSetPrimColor(OVERLAY_DISP++, 0, 0, 255, 255, 255, interfaceCtx->magicAlpha); // White color for the border
    OVERLAY_DISP =
        Gfx_TextureIA8(OVERLAY_DISP, gMagicMeterEndTex, 8, 16, debuffBarX, debuffBarY, 8, 16, 1 << 10, 1 << 10);

    // Middle part of the border (non-shrinking)
    OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gMagicMeterMidTex, 24, 16, debuffBarX + 8, debuffBarY, debuffBarWidth,
                                  16, 1 << 10, 1 << 10);

    // Right end of the border (flipped horizontally)
    gDPSetTileSize(OVERLAY_DISP++, G_TX_RENDERTILE, 0, 0, (8 - 1) << G_TEXTURE_IMAGE_FRAC,
    ...
    gSPTextureRectangle(OVERLAY_DISP++, (debuffBarX + debuffBarWidth + 8) << 2, debuffBarY << 2,
                        (debuffBarX + debuffBarWidth + 16) << 2, (debuffBarY + 16) << 2, G_TX_RENDERTILE, 256, 0,
                        1 << 10, 1 << 10);

    // --- Draw the shrinking inner bar (solid color) ---
    gDPPipeSync(OVERLAY_DISP++); // Sync to apply texture changes before switching to solid fill

    gDPSetEnvColor(OVERLAY_DISP++, colorValue, colorValue, colorValue, interfaceCtx->magicAlpha); // Fully opaque

    gDPLoadMultiBlock_4b(OVERLAY_DISP++, gMagicMeterFillTex, 0x0000, G_TX_RENDERTILE, G_IM_FMT_I, 16, 16, 0,
                         G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
                         G_TX_NOLOD);

    // Draw the solid color rectangle for the shrinking bar
    gSPTextureRectangle(OVERLAY_DISP++, (debuffBarX + 8) << 2, (debuffBarY + 3) << 2,
                        (debuffBarX + 8 + filledWidth) << 2, (debuffBarY + 13) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10,
                        1 << 10);

    gDPPipeSync(OVERLAY_DISP++);

    CLOSE_DISPS(play->state.gfxCtx, "../z_parameter.c", 3020);
}

void DrawDebuffBar2(PlayState* play, Player* player) {
    InterfaceContext* interfaceCtx = &play->interfaceCtx;
    s16 debuffBar2Y = SCREEN_HEIGHT - 30;    // Lower center of the screen
    s16 debuffBar2X = SCREEN_WIDTH / 2 - 50; // Centered horizontally
    s16 debuffBar2Width = 100;               // Total width of the interior fill bar
    s16 debuffBar2Height = 16;               // Height of the bar (based on texture height)

    // Calculate the width of the shrinking bar based on PlayerWebTimer
    s16 filledWidth = debuffBar2Width * (player->PlayerMadnessTimer / 255.0f); // Shrink fill based on timer
    ...
    gDPSetPrimColor(OVERLAY_DISP++, 0, 0, 255, 255, 255, interfaceCtx->magicAlpha); // White color for the border
    OVERLAY_DISP =
        Gfx_TextureIA8(OVERLAY_DISP, gMagicMeterEndTex, 8, 16, debuffBar2X, debuffBar2Y, 8, 16, 1 << 10, 1 << 10);

    // Middle part of the border (non-shrinking)
    OVERLAY_DISP = Gfx_TextureIA8(OVERLAY_DISP, gMagicMeterMidTex, 24, 16, debuffBar2X + 8, debuffBar2Y, debuffBar2Width,
                                  16, 1 << 10, 1 << 10);
    ...
    gSPTextureRectangle(OVERLAY_DISP++, (debuffBar2X + debuffBar2Width + 8) << 2, debuffBar2Y << 2,
                        (debuffBar2X + debuffBar2Width + 16) << 2, (debuffBar2Y + 16) << 2, G_TX_RENDERTILE, 256, 0,
                        1 << 10, 1 << 10);

    gDPPipeSync(OVERLAY_DISP++); 
        gDPSetCombineLERP(OVERLAY_DISP++, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, 0, 0, 0, PRIMITIVE, PRIMITIVE,
                          ENVIRONMENT, TEXEL0, ENVIRONMENT, 0, 0, 0, PRIMITIVE);
    gDPSetPrimColor(OVERLAY_DISP++, 0, 0, 55, 0, 100, interfaceCtx->magicAlpha); // Fully opaque
    if (((play->gameplayFrames % 2) == 0) && (player->PlayerMadnessDecayTimer > 0)) {
    gDPSetPrimColor(OVERLAY_DISP++, 0, 0, 119, 0, 200, interfaceCtx->magicAlpha);
    }
    gDPLoadMultiBlock_4b(OVERLAY_DISP++, gMagicMeterFillTex, 0x0000, G_TX_RENDERTILE, G_IM_FMT_I, 16, 16, 0,
                         G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD,
                         G_TX_NOLOD);

    gSPTextureRectangle(OVERLAY_DISP++, (debuffBar2X + 8) << 2, (debuffBar2Y + 3) << 2,
                        (debuffBar2X + 8 + filledWidth) << 2, (debuffBar2Y + 9) << 2, G_TX_RENDERTILE, 0, 0, 1 << 10,
                        1 << 10);
...
void Interface_Draw(PlayState* play) {
    ...
    if (pauseCtx->debugState == 0) {
        ...
        Minimap_Draw(play);
        if (player->PlayerWebTimer > 0) {
            DrawDebuffBar(play, player);
        }
        if (player->PlayerMadnessTimer > 0) {
            DrawDebuffBar2(play, player);
        }
