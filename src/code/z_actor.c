...
void Actor_Init(Actor* actor, PlayState* play) {
    ...
    if (BgCheck_EntityRaycastDown3(&play->colCtx, &actor->floorPoly, &bgId, &pos) != BGCHECK_Y_MIN)
...
void Actor_Draw(PlayState* play, Actor* actor) {
    ...
    if (play->envCtx.lightMode != LIGHT_MODE_TIME) {
        if (actor->category != ACTORCAT_PLAYER) {
            struct ActorLightSettings* data = &actor->lightSettings;

            u8 polySetting = 0;

            if (actor->floorPoly)
                polySetting = SurfaceType_GetLightSetting(&play->colCtx, actor->floorPoly, actor->floorBgId);

            if (data->nextSetting != polySetting) {
                data->nextSetting = polySetting;
                data->lerp = 20;
            }
            if (data->currentSetting != data->nextSetting) {
                EnvLightSettings* cur = &play->envCtx.lightSettingsList[data->currentSetting];
                EnvLightSettings* next = &play->envCtx.lightSettingsList[data->nextSetting];
                f32 mod = 1.0f - (data->lerp / 20.0f);

                for (u32 i = 0; i < 3; i++) {
                    lights->l.a.l.col[i] = lights->l.a.l.colc[i] =
                        LERP(cur->ambientColor[i], next->ambientColor[i], mod);
                    lights->l.l[0].l.dir[i] = LERP(cur->light1Dir[i], next->light1Dir[i], mod);
                    lights->l.l[1].l.dir[i] = LERP(cur->light2Dir[i], next->light2Dir[i], mod);
                    lights->l.l[0].l.col[i] = lights->l.l[0].l.colc[i] =
                        LERP(cur->light1Color[i], next->light1Color[i], mod);
                    lights->l.l[1].l.col[i] = lights->l.l[1].l.colc[i] =
                        LERP(cur->light2Color[i], next->light2Color[i], mod);
                }

                if (--data->lerp == 0)
                    data->currentSetting = data->nextSetting;
            } else {
                EnvLightSettings* cur = &play->envCtx.lightSettingsList[data->currentSetting];

                for (u32 i = 0; i < 3; i++) {
                    lights->l.l[0].l.dir[i] = cur->light1Dir[i];
                    lights->l.l[1].l.dir[i] = cur->light2Dir[i];
                    lights->l.a.l.col[i] = lights->l.a.l.colc[i] = cur->ambientColor[i];
                    lights->l.l[0].l.col[i] = lights->l.l[0].l.colc[i] = cur->light1Color[i];
                    lights->l.l[1].l.col[i] = lights->l.l[1].l.colc[i] = cur->light2Color[i];
                }
            }
        }
    }
    ...
    if ((HREG(64) != 1) || (HREG(72) != 0)) {
        if (play->actorCtx.lensActive) {
            ...
            if ((play->csCtx.state != CS_STATE_IDLE) || Player_InCsMode(play)) {
                // Actor_DisableLens(play); Applebees
            }
        }
    }
