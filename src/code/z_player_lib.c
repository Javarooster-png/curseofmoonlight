...
// Only works with upper and lower case alphanumeric characters
bool PlayerNameEquals(const char *str)
{
    const char *name = gSaveContext.save.info.playerData.playerName;
    int nameLen = ARRAY_COUNT(gSaveContext.save.info.playerData.playerName) - 1;
    
    while (name[nameLen] == 0x3E)
        nameLen -= 1;
    nameLen += 1;
    
    while (nameLen && *name != 0x3E && *str != '\0')
    {
        if (*name + 0x37 != *str // uppercase
            && *name + 0x3D != *str // lowercase
            && *name + '0' != *str // digits
        )
            return false;
    ...
    return ((*name == 0x3E || nameLen == 0) && *str == '\0');
}
...
Gfx* sPlayerRightHandShieldDLs[PLAYER_SHIELD_MAX * 4] = {
    // PLAYER_SHIELD_NONE
    gLinkAdultRightHandClosedNearDL_mesh,
    ...
    gLinkAdultRightHandHoldingHylianShieldNearDL_mesh,
    gLinkChildRightHandClosedNearDL,
...
Gfx* gPlayerLeftHandClosedDLs[] = {
    gLinkAdultLeftHandClosedNearDL_mesh,
...
Gfx* sPlayerLeftHandSwordDLs2[] = {
    gLinkAdultLeftHandHoldingMasterSwordNearDL_mesh,
...
Gfx* sPlayerRightHandOpenDLs[] = {
    gLinkAdultRightHandNearDL_mesh,
...
Gfx* sPlayerRightHandBowSlingshotDLs[] = {
    gLinkAdultRightHandHoldingBowNearDL_mesh,
...
Gfx* sPlayerRightHandHookshotDLs[] = {
    gLinkAdultRightHandHoldingHookshotNearDL_mesh,
    gLinkChildRightHandNearDL,
    gLinkAdultRightHandHoldingHookshotNearDL_mesh, // The 'far' display list exists but is not used
    gLinkChildRightHandFarDL,
};
...
Gfx* sFirstPersonLeftForearmDLs[] = {
    gLinkAdultRightArmOutNearDL_mesh,
    NULL,
};

Gfx* sFirstPersonLeftHandDLs[] = {
    gLinkAdultRightHandOutNearDL_mesh,
    NULL,
};
...
s32 Player_OverrideLimbDrawGameplayCommon(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    if (limbIndex == PLAYER_LIMB_HEAD && PlayerNameEquals("ZeldaFan")) {
        *dList = gDogHeadLinkDL;
    }

    if (limbIndex == PLAYER_LIMB_SHEATH && !CHECK_OWNED_EQUIP(EQUIP_TYPE_SWORD, EQUIP_INV_SWORD_MASTER)) {
        static Gfx blankDL[] = {gsSPEndDisplayList()};
        *dList = blankDL;        
    }

/*     if (limbIndex == PLAYER_LIMB_COLLAR && PlayerNameEquals("ZeldaFan")) {
        static Gfx blankDL[] = {gsSPEndDisplayList()};
        *dList = blankDL;
    } */
...
s32 Player_OverrideLimbDrawGameplayDefault(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    if (!Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, thisx)) {
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            ...
            *dList = *(dLists + sDListsLodOffset);
        } else if (limbIndex == PLAYER_LIMB_SHEATH && gSaveContext.save.info.equips.buttonItems[0] == ITEM_SWORD_MASTER) {
...
f32 sMeleeWeaponLengths[] = {
    ...
    3000.0f, // Master Sword
    3000.0f, // Kokiri Sword
    4000.0f, // Biggoron's Sword
    0.0f,    // Deku Stick
    2500.0f, // Hammer
};
