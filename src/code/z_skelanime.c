...
Gfx* SkelAnime_DrawChad(PlayState* play, SkelAnime* skelAnime, OverrideLimbDraw overrideLimbDraw, PostLimbDraw postLimbDraw, void* this, s16 alpha, u32 preCommandCount) {
    static Gfx yoinkSetup[] = {
        gsDPSetRenderMode(G_RM_FOG_SHADE_A, G_AC_DITHER | AA_EN | Z_CMP | Z_UPD | IM_RD | CLR_ON_CVG | CVG_DST_WRAP | ZMODE_XLU | FORCE_BL | GBL_c2(G_BL_CLR_IN, G_BL_A_IN, G_BL_CLR_MEM, G_BL_1MA)),
        gsSPEndDisplayList(),
    };
    Gfx* commands;
    
    OPEN_DISPS(play->state.gfxCtx, __FUNCTION__, __LINE__);
    
    if ((commands = preCommandCount ? Graph_Alloc(__gfxCtx, sizeof(Gfx[preCommandCount + 1])) : NULL)) {
        for (u32 i = 0; i < preCommandCount; i++)
            commands[i] = (Gfx)gsSPNoOp(); 
        commands[preCommandCount] = (Gfx)gsSPEndDisplayList();
    }
    ...
    if (alpha == 255) {
        ...
        gSPSegment(POLY_OPA_DISP++, 0x0C, yoinkSetup + 1);
        if (commands) gSPDisplayList(POLY_OPA_DISP++, commands);
        POLY_OPA_DISP = SkelAnime_DrawFlex(play, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, overrideLimbDraw, postLimbDraw, this, POLY_OPA_DISP);
    } else {
        Gfx_SetupDL_25Xlu(play->state.gfxCtx);
        gDPSetEnvColor(POLY_XLU_DISP++, 0, 0, 0, alpha);
        gSPSegment(POLY_XLU_DISP++, 0x0C, yoinkSetup);
        if (commands) gSPDisplayList(POLY_XLU_DISP++, commands);
        POLY_XLU_DISP = SkelAnime_DrawFlex(play, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, overrideLimbDraw, postLimbDraw, this, POLY_XLU_DISP);
    }
    
    CLOSE_DISPS(play->state.gfxCtx, __FUNCTION__, __LINE__);
