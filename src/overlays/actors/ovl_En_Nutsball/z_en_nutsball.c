...
void EnNutsball_ProjectileSpawn(EnNutsball* this, PlayState* play);
void EnNutsball_ProjectileFlightSettings(EnNutsball* this, PlayState* play);
...
void EnNutsball_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->requiredObjectSlot < 0) {
        ...
        this->actionFunc = EnNutsball_ProjectileSpawn;
    }
}
...
void EnNutsball_ProjectileSpawn(EnNutsball* this, PlayState* play) {
    if (Object_IsLoaded(&play->objectCtx, this->requiredObjectSlot)) {
        ...
        this->timer = 30;
        this->actionFunc = EnNutsball_ProjectileFlightSettings;
...
if (this->actor.bgCheckFlags & (BGCHECKFLAG_WALL | BGCHECKFLAG_GROUND)) {
    // Projectile hit the wall or ground
    sp40.x = this->actor.world.pos.x;
    sp40.y = this->actor.world.pos.y + 4;
    sp40.z = this->actor.world.pos.z;
    ...
    sp40.y = this->actor.world.pos.y + 4;
    sp40.z = this->actor.world.pos.z;
    player->PlayerWebTimer = 240;
    EffectSsHahen_SpawnBurst(play, &sp40, 6.0f, 0, 7, 3, 15, HAHEN_OBJECT_DEFAULT, 10, NULL);
    SfxSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 20, NA_SE_EN_OCTAROCK_ROCK);
    Actor_Kill(&this->actor);
} else if ((this->collider.base.acFlags & AC_HIT) || (this->collider.base.ocFlags1 & OC1_HIT)) {
    // Handle other collision cases here, if needed
}
}
void EnNutsball_Update(Actor* thisx, PlayState* play) {
    EnNutsball* this = (EnNutsball*)thisx;
    ...
    if (!(player->stateFlags1 & (PLAYER_STATE1_6 | PLAYER_STATE1_7 | PLAYER_STATE1_28 | PLAYER_STATE1_29)) ||
        (this->actionFunc == EnNutsball_ProjectileSpawn) || (this->actionFunc == EnNutsball_ProjectileSpawnWeb)) {
