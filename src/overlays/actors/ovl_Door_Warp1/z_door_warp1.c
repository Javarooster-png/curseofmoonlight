...
void DoorWarp1_AdultWarpOut(DoorWarp1* this, PlayState* play) {
    ...
    if (this->warpTimer > sWarpTimerTarget && gSaveContext.nextCutsceneIndex == 0xFFEF) {
        if (play->sceneId == SCENE_CATHEDRALBOSS) {
            if (!GET_EVENTCHKINF(EVENTCHKINF_48)) {
                SET_EVENTCHKINF(EVENTCHKINF_48);
                gSaveContext.save.entranceIndex = ENTR_CATHEDRALBOSS_0;
                gSaveContext.nextCutsceneIndex = 0;

            } else {
                if (!LINK_IS_ADULT) {
                    play->nextEntranceIndex = ENTR_CATHEDRALBOSS_0;
                } else {
                    play->nextEntranceIndex = ENTR_CATHEDRALBOSS_0;
                }
                gSaveContext.nextCutsceneIndex = 0;
...
#define SCENE_SHADOW_TEMPLE_BOSS 0
        } else if (play->sceneId == SCENE_CATHEDRALBOSS) {
            if (!CHECK_QUEST_ITEM(QUEST_MEDALLION_SHADOW)) {
                play->nextEntranceIndex = ENTR_HYRULE_FIELD_0;
                gSaveContext.nextCutsceneIndex = 3;
            } else {
                if (!LINK_IS_ADULT) {
