...
void EnIceHono_InitSmallFlame(Actor* thisx, PlayState* play) {
    ...
    this->actor.scale.x = this->actor.scale.z = this->actor.scale.y = 0.0014f;
    this->actor.gravity = -0.3f;
    this->actor.minVelocityY = -4.0f;
    this->actor.shape.yOffset = 0.0f;
...
void EnIceHono_InitSmallFlame2(Actor* thisx, PlayState* play) {
    EnIceHono* this = (EnIceHono*)thisx;
...
void EnIceHono_Init(Actor* thisx, PlayState* play) {
    ...
    switch (this->actor.params) {
        ...
        case 1:
            ...
            break;
        case 2:
            EnIceHono_InitSmallFlame2(&this->actor, play);
            break;
    }
...
void EnIceHono_DropFlame(EnIceHono* this, PlayState* play) {
u32 bgFlag = this->actor.bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_WALL);

    Math_StepToF(&this->actor.scale.x, 0.0017f, 0.008f);
    this->actor.scale.z = this->actor.scale.x;
    Math_StepToF(&this->actor.scale.y, 0.0017f, 0.008f);
    ...
    Collider_UpdateCylinder(&this->actor, &this->collider);
    this->collider.dim.radius = this->actor.scale.x * 12000.0f;
    this->collider.dim.height = this->actor.scale.y * 16000.0f;
    CollisionCheck_SetOC(play, &play->colChkCtx, &this->collider.base);
...
void EnIceHono_SpreadFlames(EnIceHono* this, PlayState* play) {
    if (this->timer > 20) {
        Math_StepToF(&this->actor.scale.x, 1.0f, 0.00014f);
        Math_StepToF(&this->actor.scale.y, 0.1f, 0.00012f);
    } else {
    ...
    //   if (PLAYER_IA_DEKU_NUT) {
    //       EnIceHono_Destroy(this, play);
    // }
}
...
void EnIceHono_SetupActionSmallFlame(EnIceHono* this) {
    ...
    if (this->actor.params == 1) {
        this->smallFlameTargetYScale = (Rand_ZeroOne() * 0.01f) + 0.01f;
        this->actor.speed = (Rand_ZeroOne() * 1.0f) + 1.5f;
    } else {
        this->smallFlameTargetYScale = (Rand_ZeroOne() * 0.06f) + 0.01f;
        this->actor.speed = (Rand_ZeroOne() * 3.0f) + 1.7f;
    }
}
...
void EnIceHono_SmallFlameMove(EnIceHono* this, PlayState* play) {
    ...
    if ((this->timer < 30) && (this->actor.params == 2)) {
        this->alpha -= 20;
        this->alpha = CLAMP(this->alpha, 0, 255);
    }
    if (this->timer <= 0) {
