...
void BossGanon_CutsceneStuff(BossGanon2* this, PlayState* play);
...
void BossGanon2_Init(Actor* thisx, PlayState* play) {
    ...
    Collider_SetJntSph(play, &this->colliderSpheres, &this->actor, &sJntSphInit1, this->colliderSphereElements);
    Collider_InitJntSph(play, &this->colliderSpheres2);
    Collider_SetJntSph(play, &this->colliderSpheres2, &this->actor, &sJntSphInit2, this->colliderSphereElements2);
...
void BossGanon2_Destroy(Actor* thisx, PlayState* play) {
    ...
    Collider_DestroyJntSph(play, &this->colliderSpheres2);
}
...
void func_80900650(BossGanon2* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, this->unk_194)) {
        ...
        if ((this->unk_311 == 1) && (this->actor.xzDistToPlayer < 250.0f) && this->look_on) {
            func_80900580(this, play);
        } else {
            func_808FFDB0(this, play);
        }
...
void func_80900818(BossGanon2* this, PlayState* play) {
    ...
    this->actionFunc = BossGanon_CutsceneStuff;
    this->unk_1AC = 0;
    this->unk_39C = 0;
...
void func_80902348(BossGanon2* this, PlayState* play) {
    ...
    if (this->no_hit_time == 0) {
        for (i = 0; i < ARRAY_COUNT(this->colliderSphereElements2); i++) {
            if (this->colliderSpheres2.elements[i].info.bumperFlags & BUMP_HIT) {
                this->colliderSpheres2.elements[i].info.bumperFlags &= ~BUMP_HIT;
            } else if (this->colliderSpheres2.elements[i].info.toucherFlags & TOUCH_HIT) {
                this->colliderSpheres2.elements[i].info.toucherFlags &= ~TOUCH_HIT;
...
void BossGanon2_HandleDamage(BossGanon2* this, PlayState* play) {
    s8 temp_v0_4;
    ...
    if (this->no_hit_time != 0 || ((this->unk_334 == 0) && (this->actionFunc == BossGanon_CutsceneStuff))) {
        for (i = 0; i < ARRAY_COUNT(this->colliderSphereElements); i++) {
    ...
    if (this->look_on) {
        if (this->actionFunc != func_808FFFE0) {
            if (this->colliderSpheres.elements[0].info.bumperFlags & BUMP_HIT) {
                this->colliderSpheres.elements[0].info.bumperFlags &= ~BUMP_HIT;
                acHitInfo = this->colliderSpheres.elements[0].info.acHitInfo;
                if ((acHitInfo->toucher.dmgFlags & DMG_ARROW_LIGHT) && (this->actionFunc != BossGanon_CutsceneStuff)) {
                    func_809000A0(this, play);
                    Actor_PlaySfx(&this->actor, NA_SE_EN_FANTOM_HIT_THUNDER);
                    Actor_PlaySfx(&this->actor, NA_SE_EN_MGANON_DAMAGE);
                    Audio_StopSfxById(NA_SE_EN_MGANON_UNARI);
                } else if ((this->actionFunc == BossGanon_CutsceneStuff) &&
        ...
        if (this->colliderSpheres.elements[15].info.bumperFlags & BUMP_HIT) {
            this->colliderSpheres.elements[15].info.bumperFlags &= ~BUMP_HIT;
            acHitInfo = this->colliderSpheres.elements[15].info.acHitInfo;
            this->no_hit_time = 60;
            this->unk_344 = 0x32;
            this->unk_342 = 5;
            Actor_PlaySfx(&this->actor, NA_SE_EN_MGANON_DAMAGE);
...
void BossGanon2_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != func_8090120C) {
        BossGanon2_HandleDamage(this, play);
        CollisionCheck_SetAC(play, &play->colChkCtx, &this->colliderSpheres.base);
        CollisionCheck_SetOC(play, &play->colChkCtx, &this->colliderSpheres2.base);
        CollisionCheck_SetAC(play, &play->colChkCtx, &this->colliderSpheres2.base);
        if (this->subCamId == SUB_CAM_ID_DONE) {
            CollisionCheck_SetAT(play, &play->colChkCtx, &this->colliderSpheres2.base);
        }
    }
...
void BossGanon2_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (temp_v0 >= 0) {
        ...
        func_808FD080(temp_v0, &this->colliderSpheres, &sp4C);
    }
    ...
    if (*dList != NULL) {
        if ((limbIndex == GANON_LIMB_LEFT_SWORD) && (this->unk_312 == 1)) {
            ...
            func_808FD080(0, &this->colliderSpheres2, &this->unk_218);
            ...
            Matrix_MultVec3f(&D_809070FC, &this->unk_218);
            func_808FD080(1, &this->colliderSpheres2, &this->unk_218);
            Matrix_MultVec3f(&D_80907120, &this->unk_200);
            Matrix_MultVec3f(&D_8090712C, &this->unk_20C);
        }
    }
...
void BossGanon2_Draw(Actor* thisx, PlayState* play) {
    ...
    switch (this->unk_337) {
        ...
        case 2:
            ...
            func_808FD080(0, &this->colliderSpheres2, &D_8090717C);
            func_808FD080(1, &this->colliderSpheres2, &D_8090717C);
